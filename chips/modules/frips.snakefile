...
rule sample_4M_from_nonChrM:
    ...
    log:_logfile
    output:
        'analysis/align/{sample}/{sample}_4M_nonChrM.bam' # keeping this non-temp while developing so we can easily add on enrichment, etc.
    shell:
        """
...
rule promoter_reads:
    """Calculate the fraction of reads near promoters"""
    input:
        treat=frip_getTreatBam,
    output:
        "analysis/frips/{run}.{rep}/{run}.{rep}_fr_promoter_reads.txt"
    params:
        bed=config['promoters']
    message: "calculate fraction of reads in promoters"
    log:_logfile
    shell:
...
rule enrichment:
    ...
    input:
        "analysis/align/{run}/{run}_unique.sorted.dedup.bam"
    output:
        "analysis/frips/{run}.{rep}/{run}.{rep}_enrichment.txt"
    params:
        targets=config['enrichment_targets']
    message: "calculate on target enrichment"
    log:_logfile
    shell:
        "sh chips/modules/scripts/enrichment.sh {input} {params.targets} {wildcards.run} > {output} 2>>{log}"
...
rule getPromoterReadStats:
    """Collect the promoter reads statistics from analysis/frips/{run}/{run}_fr_promoter_reads.txt"""
    input:                                                                           
        #Generalized INPUT fn defined in chips.snakefile                             
        _getRepInput("analysis/frips/$runRep/$runRep_fr_promoter_reads.txt")                      
    output:                                                                          
        "analysis/frips/fr_promoter_reads.csv"                                                   
    message: "FRiPs: collecting promoter read stats for each run"                            
    log:_logfile                                                                     
    run:                                                                             
        files = " -f ".join(input)                                                   
        shell("chips/modules/scripts/frips_getFrips.py -f {files} -o {output} 2>>{log}")
